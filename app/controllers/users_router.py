...
from app.dto.users import UserIn, UserOut, LoginRequest
from app.models.users import User
from pwdlib import PasswordHash

from app.dto.base import ReponseWrapper

from app.utils.auth import verify_password, create_access_token, create_refresh_token, hash_password, get_current_user, verify_refresh_token
...
async def update_user(user_id: str, data: UserIn, current_user: str = Depends(get_current_user)):
  try:
    user = await User.get(user_id)
    if not user:
      raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    elif str(user._id) != current_user:
      raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to update this user")
    data.password = hash_password(data.password) if data.password else user.password
    await user.set(data.model_dump())
...
@router.get("/me", response_model=ReponseWrapper[UserOut], description="Get current user", status_code=status.HTTP_200_OK)
